...
void recv_cb(const struct device *dev, uint8_t *data, uint16_t size, int16_t rssi, int8_t snr)
{
	...
	struct pkg_data_actuator *pkg_data;
	...
	pkg_data = (struct pkg_data_actuator *)&msg->data;
	...
	crc = crc32_ieee((uint8_t *)pkg_data, msg->header.data_len);
	if (crc != msg->header.crc) {
		LOG_DBG("Invalid crc | got: %d | expected: %d\n", crc, msg->header.crc);
		return;
	}

	LOG_INF("output0: %d output1: %d\n", pkg_data->want.outputs[0], pkg_data->want.outputs[1]);

	zbus_chan_pub(&output_chan, &pkg_data->want, K_MSEC(200));
}
